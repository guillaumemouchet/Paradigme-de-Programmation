...
class Sas1
	...
	public void entrerDansSas(int qui) throws InterruptedException
		...
		try
			...
			// On doit retester car si on est 6 à attendre, il y en a que 5 qui passe
			while(!remplir)
				{
				entrer.await();
				}
			...
			if (contenu == 0)
				...
				remplir = false;
				sortir.signalAll(); //sas plein, réveil cote sortie
				}
			else
				{
	...
	public void sortirDuSas(int qui) throws InterruptedException
		...
		try
			...
			if (contenu == capacite)
				...
				remplir = true;
				entrer.signalAll(); //sas vide, réveil coté entree
				}
			else
				{
