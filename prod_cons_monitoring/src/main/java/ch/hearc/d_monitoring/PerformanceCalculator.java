...
public class PerformanceCalculator {
    ...
    public static String getReport() {
        ArrayList<Long> sentTimes = PerformanceMeasures.getInstance().getMessageSentTimes();
        ArrayList<Long> shownTimes = PerformanceMeasures.getInstance().getMessageShownTimes();
        ...
        if (sentTimes.size() >= shownTimes.size()) {
            latencyNs = IntStream.range(0, shownTimes.size())
                    .mapToDouble(i -> (shownTimes.get(i) - sentTimes.get(i)))
                    .average();
        } else {
            latencyNs = IntStream.range(0, sentTimes.size())
                    .mapToDouble(i -> (shownTimes.get(i) - sentTimes.get(i)))
                    .average();
        }
        ...
        latencyMs = latencyNs.getAsDouble() / (double)1000000; // milliseconds
        ...
        stringBuilder.append("Effective throughput [Mb/s]: ").append(tabs(1)).append(round(throughputBPerS / (double)1000000)).append("\n");
    ...
    private static double averageMessageSizeInBytes() {
        ArrayList<String> messages = PerformanceMeasures.getInstance().getMessagesShown();
        OptionalDouble averageBytes =  messages.stream()
                .mapToDouble(message -> (double)message.getBytes().length)
                .average();
        if (averageBytes.isEmpty()) {
            throw new RuntimeException("The average message size could not be calculated");
        }
        return averageBytes.getAsDouble();
    }
